...
public:
	...
	void ImportTerrain(const std::string& FileName, CTerrain::STerrainFileData& Data, vector<unique_ptr<CObject3D>>& vFoliageObject3Ds,
		ID3D11Device* const PtrDevice, ID3D11DeviceContext* const PtrDeviceContext, CGame* const PtrGame);
	void ExportTerrain(const std::string& FileName, const CTerrain::STerrainFileData& Data, const vector<unique_ptr<CObject3D>>& vFoliageObject3Ds);
