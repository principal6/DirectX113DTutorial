...
void main(triangle VS_OUTPUT Input[3], inout TriangleStream<VS_OUTPUT> Output)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		Vertex.Position = mul(Vertex.WorldPosition + normalize(Input[i].WorldNormal) * LINE_LENGTH, ViewProjection);
		Output.Append(Vertex);
		Output.Append(Vertex);
	...
	for (int j = 0; j < 3; ++j)
	{
		...
		Vertex.Position = mul(Vertex.WorldPosition + normalize(Input[j].WorldTangent) * LINE_LENGTH, ViewProjection);
		Output.Append(Vertex);
		Output.Append(Vertex);
	...
	for (int k = 0; k < 3; ++k)
	{
		...
		Vertex.Position = mul(Vertex.WorldPosition + normalize(Input[k].WorldBitangent) * LINE_LENGTH, ViewProjection);		
		Output.Append(Vertex);
		Output.Append(Vertex);
