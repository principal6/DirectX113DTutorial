...
#define LightWorldPosition Input.WorldPosition
#define LightDirection Input.Direction

float4 main(DS_SPOT_LIGHT_OUTPUT Input) : SV_TARGET
{
	...
	float4 OutputColor = float4(0, 0, 0, 1);
	{
		...
		float CosAngle = dot(LightDirection.xyz, -Wi);
		float CosAngleThreshold = Input.CosThetaDiv2;
		if (CosAngle > CosAngleThreshold)
		{
			float AngleAttenuation = CosAngle - CosAngleThreshold; // [0, 1.0 - CosAngleThreshold]
			AngleAttenuation /= (1.0 - CosAngleThreshold); // [0, 1]
			AngleAttenuation *= AngleAttenuation;

			float DistanceAttenuation = saturate(1.0 - DistanceToLight * Input.InverseRange);
			DistanceAttenuation *= DistanceAttenuation;

			OutputColor.xyz = Lo * DistanceAttenuation * AngleAttenuation;
		}
	}
...
float4 Volume(DS_SPOT_LIGHT_OUTPUT Input) : SV_TARGET
{
	return float4(0.5, 1.0, 0.0, 1.0);
}