...
void CObject3DLine::Create(const vector<SVertex3DLine>& _vVertices)
{
	...
	BufferDesc.ByteWidth = static_cast<UINT>(sizeof(SVertex3DLine) * vVertices.size());
...
void CObject3DLine::Update()
{
	...
	{
		memcpy(MappedSubresource.pData, &vVertices[0], sizeof(SVertex3DLine) * vVertices.size());
