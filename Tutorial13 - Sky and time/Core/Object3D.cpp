...
void CObject3D::UpdateQuadUV(const XMFLOAT2& UVOffset, const XMFLOAT2& UVSize)
{
	...
	UpdateQuadMeshBuffer();
}

void CObject3D::UpdateQuadMeshBuffer()
{
	D3D11_MAPPED_SUBRESOURCE MappedSubresource{};
	if (SUCCEEDED(m_PtrDeviceContext->Map(m_vMeshBuffers[0].VertexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedSubresource)))
	{
		memcpy(MappedSubresource.pData, &m_Model.vMeshes[0].vVertices[0], sizeof(SVertex3D) * m_Model.vMeshes[0].vVertices.size());

		m_PtrDeviceContext->Unmap(m_vMeshBuffers[0].VertexBuffer.Get(), 0);
	}
}
